"""
Resume X-Ray Ã¢ÂÂ Fintech Edition (v1)
...
and surfaces hidden rejection reasons Ã¢ÂÂ before you apply.
...
# ============================================================
# SKILL SYNONYMS Ã¢ÂÂ word-boundary matched so "r" doesn't match
...
def has_word(needle: str, haystack: str) -> bool:
    ...
    if " " not in needle_lower and "-" not in needle_lower:
        # Single word Ã¢ÂÂ straight word-boundary match
...
def extract_years_experience(text: str) -> int:
    ...
    Strategy:
    1. Try explicit 'X+ years ...' patterns first Ã¢ÂÂ most reliable signal.
       Catches: 'X years of experience', 'X+ years in [domain]',
                'over X years', 'X years building/leading/managing'.
    2. Fall back to summing year ranges. Handles both 'YYYY Ã¢ÂÂ YYYY'
       and 'Month YYYY Ã¢ÂÂ Month YYYY' formats.
    """
    ...
    explicit_patterns = [
        ...
        # 'X+ years <verb/preposition>' Ã¢ÂÂ catches '10 years applying analytical methods',
    ...
    # end year handles 'Feb 2014 Ã¢ÂÂ Jul 2024' style (skips 'Jul ' to find 2024).
    year_range = r'(20\d{2}|19\d{2})\s*[-Ã¢ÂÂ]\s*(?:[a-zA-Z]+\.?\s+)?(20\d{2}|19\d{2}|present|current)'
...
# ============================================================
# SCORING ENGINE Ã¢ÂÂ 5 dimensions
# ============================================================
...
def score_keyword_narrative_balance(candidate: CandidateProfile) -> DimensionScore:
    ...
    if total_words < 50:
        return DimensionScore(
            ...
            gaps=["Resume may be image-heavy or too brief Ã¢ÂÂ extract text-based PDF"]
        )
...
# ============================================================
# HIDDEN REJECTION DETECTOR Ã¢ÂÂ the killer feature
# ============================================================
...
# ============================================================
# TOP FIXES Ã¢ÂÂ deterministic, ranked by lowest dimension scores
# ============================================================
    ...
    for dim_name, dim_score in ranked[:3]:
        ...
        if dim_name == "skill_match":
            ...
            fix["action"] = (
                ...
                f"Path: tutorial Ã¢ÂÂ portfolio project Ã¢ÂÂ resume bullet."
            )
...
# ============================================================
# VERDICT Ã¢ÂÂ single Groq call with deterministic fallback
# ============================================================
...
def score_emoji(s: int) -> str:
    if s >= 75: return "Ã°ÂÂÂ¢"
    if s >= 50: return "Ã°ÂÂÂ¡"
    return "Ã°ÂÂÂ´"
...
def build_report_markdown(
    ...
    if candidate.years_experience > exp["max_years"] + 2:
        ...
        if level == "Junior":
            ...
            suggested = "Senior"
        mismatch_banner = (
            f"\n> Ã¢ÂÂ Ã¯Â¸Â **Heads up Ã¢ÂÂ possible level mismatch.** You selected **{level}** "
            f"(typical {exp['min_years']}Ã¢ÂÂ{exp['max_years']} yrs) but your resume "
    ...
    md = f"""# Ã°ÂÂ©Â» Resume X-Ray Report
...
## Ã°ÂÂÂ Diagnostic Breakdown

### 1. Skill Match Ã¢ÂÂ {score_emoji(scores['skill_match'].score)} {scores['skill_match'].score}/100
"""
    ...
    md += f"\n### 2. Seniority Signals ({level} bar) Ã¢ÂÂ {score_emoji(scores['seniority'].score)} {scores['seniority'].score}/100\n"
    ...
    md += f"\n### 3. Business Impact Ã¢ÂÂ {score_emoji(scores['impact'].score)} {scores['impact'].score}/100\n"
    ...
    md += f"\n### 4. Keyword vs Narrative Balance Ã¢ÂÂ {score_emoji(scores['narrative'].score)} {scores['narrative'].score}/100\n"
    ...
    md += f"\n### 5. Modern Stack Ã¢ÂÂ {score_emoji(scores['modern_stack'].score)} {scores['modern_stack'].score}/100\n"
    ...
    md += "\n---\n\n## Ã°ÂÂÂ¨ Hidden Rejection Reasons\n\n"
    if rejections:
        md += "*The silent reasons recruiters pass on resumes Ã¢ÂÂ even when keywords match.*\n\n"
        for i, r in enumerate(rejections, 1):
            md += f"**{i}.** {r}\n\n"
    else:
        md += "Ã¢ÂÂ No major hidden rejection signals detected. Your resume should clear initial screens.\n"

    md += "\n---\n\n## Ã°ÂÂÂ  Top Fixes (Priority Order)\n\n"
    if top_fixes:
...
# ============================================================
# ============================================================
# USAGE LOGGING Ã¢ÂÂ append a row to usage_log.csv each run AND
...
def run_xray(resume_file, role: str, level: str, location: str, previous_state=None):
    ...
    if resume_file is None:
        return "Ã¢ÂÂ **Please upload your resume (PDF format).**", None

    if role not in FINTECH_ROLES:
        return f"Ã¢ÂÂ Role '{role}' not supported in v1. Available: {list(FINTECH_ROLES.keys())}", None

    if level not in SENIORITY_EXPECTATIONS:
        return "Ã¢ÂÂ Level must be Junior, Mid, or Senior.", None

    candidate = parse_resume(resume_file.name)
    if not candidate.raw_text:
        return "Ã¢ÂÂ Could not read resume. Make sure it's a text-based PDF (not a scanned image).", None
...
def submit_feedback(feedback_text: str, role: str, level: str, location: str, pending_state):
    ...
    Returns (status_message, new_state) Ã¢ÂÂ new_state cleared after submission.
    """
    ...
    if not feedback_text:
        return "Ã¢ÂÂ Ã¯Â¸Â Please write some feedback before sending.", pending_state
    ...
    if pending_state:
        ...
        # Feedback without an X-Ray run Ã¢ÂÂ empty scores
        row = {
            "timestamp": datetime.now().isoformat(timespec="seconds"),
        ...
        print(f"[submit_feedback] no pending state Ã¢ÂÂ posting feedback-only row")

    _write_csv_row(row)
    _post_to_form_async(row)

    return "Ã¢ÂÂ Thanks Ã¢ÂÂ your feedback was received. It helps me make this tool better.", None
...
def build_ui():
    with gr.Blocks(
        ...
        gr.Markdown("""
        # 🩻 Resume X-Ray
        ### Find out why your resume is getting rejected â before you apply.
        ...
        *5 diagnostic dimensions. One honest verdict. ~10 seconds.*
        ...
        with gr.Row():
            ...
            with gr.Column(scale=2):
                ...
                gr.Markdown(
                    ...
                    "Type below and hit Send. Anonymous â no email captured.*"
                )
        ...
        with gr.Accordion("âï¸ API Status", open=False):
            gr.Markdown(
        ...
        # -------------------------------------------------------
        def set_loading():
            return "â³ **Analyzing your resume\u2026 this takes ~10 seconds. Please wait.**", ""
...
if __name__ == "__main__":
    print("=" * 60)
    print("🩻 RESUME X-RAY v1 â Fintech Edition")
    print("=" * 60)
    print(f"Roles: {len(FINTECH_ROLES)} | Levels: {len(SENIORITY_EXPECTATIONS)}")
    print(f"Groq API: {'â' if GROQ_API_KEY else 'â'}")
    print("=" * 60)
    demo = build_ui()
    demo.launch()
